...
# 5 模型评估
# 线性核函数 模型评估
print("Sigmoid核函数支持向量机的默认评估值为：", linear_svr.score(x_test, y_test))
print("Sigmoid核函数支持向量机的R_squared值为：", r2_score(y_test, linear_svr_y_predict))
print("Sigmoid核函数支持向量机的均方误差MSE为:", mean_squared_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1))))
print("Sigmoid核函数支持向量机的平均绝对误差MAE为:", mean_absolute_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                                 scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1))))
print("Sigmoid核函数支持向量机的均方根误差RMSE为:", np.sqrt(mean_squared_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1)))))

print("Sigmoid核函数支持向量机的均方误差MSE为:", mean_squared_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1))))
print("Sigmoid核函数支持向量机的平均绝对误差MAE为:", mean_absolute_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                                 scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1))))
print("Sigmoid核函数支持向量机的均方根误差RMSE为:", np.sqrt(mean_squared_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1)))))
