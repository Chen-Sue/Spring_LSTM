...
print("线性核函数支持向量机的均方误差MSE为:", mean_squared_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1))))
print("线性核函数支持向量机的平均绝对误差MAE为:", mean_absolute_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                                 scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1))))
print("线性核函数支持向量机的均方根误差RMSE为:", np.sqrt(mean_squared_error(scaler_y.inverse_transform(y_train.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict_train.reshape(-1, 1)))))


print("线性核函数支持向量机的均方误差MSE为:", mean_squared_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1))))
print("线性核函数支持向量机的平均绝对误差MAE为:", mean_absolute_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                                 scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1))))
print("线性核函数支持向量机的均方根误差RMSE为:", np.sqrt(mean_squared_error(scaler_y.inverse_transform(y_test.reshape(-1, 1)),
                                              scaler_y.inverse_transform(linear_svr_y_predict.reshape(-1, 1)))))

#=================================================================================
# predict origin
# ==============
y_pre = linear_svr.predict(x_all)
predict = scaler_y.inverse_transform(y_pre.reshape(-1, 1))
origin = scaler_y.inverse_transform(y_all)
...
# coef1 = np.polyfit(predict, origin, 1)
# poly_fit1 = np.poly1d(coef1)
# plt.plot(a, poly_fit1(a), 'g',label="一阶拟合")
...
a = reg.coef_[0, 0]
b = reg.intercept_[0]
plt.text(4.6, 6.8, "y={0:.4f}x+({1:.4f})".format(a, b), fontsize=20)
plt.text(4.6, 6.5, "R$^2$ = {:.4f}".format(r2_score(predict.reshape(-1, 1), origin.reshape(-1, 1))), fontsize=20)
plt.text(2.1, 6.8, "c", fontsize=26)
plt.xlim(1.9, 7.1)
plt.ylim(1.9, 7.1)
plt.xlabel("Simulated Spring Discharge (m$^3$/s)", fontsize=22)
plt.ylabel("Observed Spring Discharge (m$^3$/s)", fontsize=22)
# plt.legend(loc="upper left", shadow=True, fontsize=15)
plt.savefig("./pictures/fit.png")
plt.savefig("./pictures/fit.eps")
...
def plot_curve(true_data, predicted_data):
	...
	plt.ylabel("Spring Discharge (m$^3$/s)", fontsize=22)
	# plt.axvline(lag+1)
	# plt.axvline(26*each_year_month)
	# plt.axvline(32*each_year_month)
	plt.text(1.0, 6.8, "c", fontsize=26)
	plt.ylim(1.9, 7.1)
	plt.yticks(fontsize=20)
	plt.xticks(rotation=30, fontsize=20)
	plt.xlim(-2, 32*12+2)
	plt.xticks(np.arange(1, 32*12+1, 1), ('1985.12', '1986.12', '1987.12', '1988.12','1989.12','1990.12','1991.12','1992.12','1993.12','1994.12','1995.12','1996.12','1997.12','1998.12','1999.12','2000.12','2001.12','2002.12','2003.12','2004.12','2005.12','2006.12','2007.12','2008.12','2009.12','2010.12','2011.12','2012.12','2013.12','2014.12','2015.12','2016.12','2017.12','2018.12', '2019.12'))
	...
	plt.legend(loc='upper right', shadow=False, fontsize=18)
	plt.grid(axis='x', which='minor')
	plt.savefig('./pictures/spring.png')
	plt.savefig("./pictures/spring.eps")
	# plt.show()
